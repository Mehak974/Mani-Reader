...
export default function MangaDetailClient({ id, initialManga, initialChapters }) {
  ...
  const [loading, setLoading] = React.useState(!initialManga || !initialChapters || initialChapters.length === 0);
  ...
  React.useEffect(() => {
    // Skip client-side fetching if we already have server-side info and chapters
    if (initialManga && initialChapters && initialChapters.length > 0) {
      setLoading(false);
      if (user) {
    ...
    mangaApi.related(id).then((r) => setRelated(r.data || [])).catch(() => { });
  }, [id, user, initialManga, initialChapters]);
